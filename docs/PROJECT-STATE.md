...
- O botao "Limpar palco" agora tambem zera o tempo decorrido do evento (o icone de parar um bloco individualmente nao zera). O formulario de mensagem (input + Temporaria/Permanente/Limpar) e o botao "Limpar palco" moraram para o card "Preview ao vivo", logo abaixo da caixa de preview, todos na mesma linha de acoes — nao ficam mais em "Evento Aberto". Evidencia em `.sdd/features/2026-09-01-reset-tempo-e-layout-preview/RECORD.md`, `.sdd/features/2026-09-01-preview-overflow-e-limpar-palco/RECORD.md` e `.sdd/features/2026-09-01-mensagem-no-card-preview/RECORD.md`.
- O card "Em breve" virou "Relatorio de blocos": tabela Nome/Tempo/Atraso por bloco, atualizada ao vivo, com atraso em vermelho quando o bloco estoura o tempo planejado (ex.: 30 min planejado, 35 usado = "-5 min") e em verde quando termina adiantado (ex.: "+10 min"); os totais somam positivos e negativos juntos. Exige a nova coluna `time_blocks.actual_seconds` (migration `0004`). "Limpar palco" zera a tabela junto com o tempo decorrido. Console de gestao ganhou tambem "Horario atual" (hh:mm:ss ao vivo) acima de "Total Planejado". Coluna "Eventos" ficou mais estreita e a de "Relatorio de blocos" mais larga. Nome do evento agora tem limite de 20 caracteres (criacao e edicao), com aviso visivel. Evidencia em `.sdd/features/2026-09-01-relatorio-blocos-atraso/RECORD.md`, `.sdd/features/2026-09-01-horario-atual/RECORD.md` e `.sdd/features/2026-09-01-layout-limite-nome-cores-atraso/RECORD.md`.
- Cada bloco ganhou um botao "Finalizar" (icone de bandeira): para o cronometro do bloco (se estiver ativo) e trava o bloco permanentemente — sem iniciar/parar, so editar e excluir, com rotulo "Finalizado" e visual apagado. Exige a nova coluna `time_blocks.finished_at` (migration `0005`). Evidencia em `.sdd/features/2026-09-01-finalizar-bloco/RECORD.md`.
- Os cards "Eventos", "Evento Aberto" e "Relatorio de Blocos" tem altura fixa (nao crescem mais com o conteudo; listas e tabela rolam internamente quando excedem o espaco). Botoes de mensagem ficaram mais curtos ("Temporaria"/"Permanente"/"Limpar", com `title` explicando cada um) para caberem numa linha so. Evidencia em `.sdd/features/2026-09-01-altura-fixa-cards/RECORD.md`.
- Layout do console de gestao (topo): Eventos, Evento Aberto e Preview ao vivo (606.55px de largura fixa) ficam lado a lado, cada um com 600px de altura fixa. Relatorio de Blocos passou para uma linha propria em largura total abaixo dos 3, crescendo livremente com o conteudo (sem altura fixa nem rolagem). A coluna "Atraso" da tabela foi renomeada para "Atraso / Adiantado". Evidencia em `.sdd/features/2026-09-01-preview-terceiro-card/RECORD.md`.
- Corrigido bug de overflow no preview: mensagem de texto (ex.: 50 caracteres) estourava a caixa do preview desde que ele passou a ter largura fixa (a fonte usava `vw`, relativo ao viewport, nao a largura do card). A tela de palco (HDMI) nunca teve esse problema e nao foi alterada. Evidencia em `.sdd/features/2026-09-01-preview-overflow-e-limpar-palco/RECORD.md`.
- Todos os botoes e inputs do console de gestao tem altura padronizada em 2.5rem (40px), incluindo "Limpar palco" e "Adicionar bloco" (que quebravam texto em 2 linhas e esticavam os inputs vizinhos). Regra global: texto de botao nunca quebra linha. Esse e outros padroes visuais foram documentados em `.sdd/knowledge/design-system.md`, a ser lido antes de criar novos elementos visuais. Evidencia em `.sdd/features/2026-09-01-padronizar-altura-controles/RECORD.md`.
- Foco de inputs/botoes trocou o outline padrao do navegador (grosso, escuro) por um destaque discreto em teal (`:focus-visible`). Padding lateral da pagina (`.management-shell`) caiu pela metade (padding vertical inalterado), dando mais espaco horizontal aos cards. Evidencia em `.sdd/features/2026-09-01-foco-discreto-e-padding/RECORD.md`.
...
- Corrigido o mini-timer (exibido junto com mensagem ativa): no palco, ele aparecia centralizado no topo em vez de no canto esquerdo (herdava estilo de uma regra generica do timer gigante); em ambas as telas, o tamanho foi aumentado por estar pequeno demais; e depois, o afastamento do canto foi ajustado (ficava colado na quina porque `position:absolute` ignora o padding do container). Evidencia em `.sdd/features/2026-09-01-timer-pequeno-com-mensagem/RECORD.md`.
- Texto da mensagem (temporaria ou permanente) agora aparece sempre em vermelho (`#ff4d45`) no palco e no preview. Evidencia em `.sdd/features/2026-09-01-mensagem-vermelha/RECORD.md`.
- Mini-timer (exibido junto com mensagem ativa) afastado ainda mais do canto: palco de `top:3rem; left:3rem` para `top:6rem; left:5rem`; preview de `top:1.25rem; left:1.25rem` para `top:2.25rem; left:2rem`. Evidencia em `.sdd/features/2026-09-01-timer-pequeno-com-mensagem/RECORD.md`.
