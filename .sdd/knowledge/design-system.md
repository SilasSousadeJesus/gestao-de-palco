...
- **Altura padrao de controles: `2.5rem` (40px), sempre.** Todo `input` e `button` novo nasce com essa altura, herdada de `input, button { min-height: 2.5rem; ... }` em `globals.css` — nao redeclare uma altura diferente por conta propria. So se afasta desse padrao quando o usuario pedir explicitamente uma medida diferente para um controle especifico (ex.: os icones de acao quadrados de `1.9rem`, ja documentados como excecao deliberada abaixo, ou uma largura/altura pontual pedida pelo usuario). Fora isso, todo botao e input segue os 2.5rem (ex.: "Criar evento", "Adicionar bloco", os inputs do formulario de bloco e os botoes de mensagem devem todos ficar do mesmo tamanho).
...
- **Uma lista em grid com poucos itens (ou so 1) nao pode ficar sem `align-content:start`.** Sem isso, o grid distribui o espaco livre entre as linhas existentes (comportamento padrao de `align-content:normal`), e com 1 item so, ele estica para ocupar o container inteiro — foi o bug corrigido em 01/09/2026 em `.event-list` e `.blocks` (um unico bloco/evento tomava a altura toda do card). Sempre que uma lista rolavel (`flex:1; min-height:0; overflow-y:auto`) tiver poucos itens, garanta `align-content:start` no container em grid.
- **Um elemento novo dentro de um container que ja tem uma regra generica por tag (`strong`, `button`, etc.) pode herdar `width`/`text-align`/`justify-self` sem querer**, mesmo com `position:absolute`. Foi o caso do `.stage-mini-timer` no palco: a caixa ficava com `width:100%` e o texto `text-align:center` herdados de uma regra mais antiga (`.stage-screen .stage-stage strong`), entao mesmo com `left:0` a caixa esticava e o texto aparecia centralizado, nao no canto. Ao posicionar um elemento novo com `position:absolute`, declare explicitamente `width`/`text-align`/`justify-self` na regra especifica dele, com especificidade maior que a regra generica — nao confie so no `top`/`left`.
- **Ao verificar posicionamento com `getBoundingClientRect()`, meça tambem `text-align`/`width` computados, nao so as bordas do elemento.** Uma caixa pode estar corretamente em `left:0` (confirmando a posicao do elemento) enquanto o texto renderizado dentro dela aparece centralizado por causa de `width:100%` + `text-align:center` herdados — foi exatamente o que aconteceu na primeira verificacao do mini-timer, que so checou a borda da caixa e concluiu (errado) que a posicao estava correta.
- **`padding` do container "positioned" nao afeta filhos com `position:absolute`.** O containing block de um elemento absoluto e a padding box do ancestral posicionado, entao `top:0; left:0` cola o elemento na quina, ignorando qualquer `padding` que o container aplique so ao fluxo normal — foi o motivo do mini-timer ficar "colado" mesmo o `.stage-stage`/`.stage-presentation` tendo padding generoso. Para dar respiro a um elemento absoluto, defina um `top`/`left` (ou `inset`) maior que zero explicitamente — nao contar com o padding do pai.
